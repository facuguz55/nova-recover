...
function buildRecuperador(clientId: string, tnStoreId: string, tnApiToken: string, nombreMarca: string, supabaseAnon: string) {
  ...
  return {
    ...
    nodes: [
      ...
      { id: "node-r-7", name: "Enviar Gmail Recuperacion", type: "n8n-nodes-base.gmail", typeVersion: 2, position: [1408, 304], credentials: { gmailOAuth2: { id: GMAIL_CREDENTIAL_ID, name: "novaagency" } }, parameters: { sendTo: "={{ $json.email_destino }}", subject: `Completaste tu carrito en ${nombreMarca}`, message: `=${emailHtml}`, options: { senderName: nombreMarca } } },
...
export async function POST(req: Request) {
  // Permite llamada interna con user_id (desde stripe webhook) o autenticada
  const supabase = await createServerClient();
  const { data: { user } } = await supabase.auth.getUser();

  // También acepta user_id en el body para llamadas internas
  ...
  if (!onboarding?.tn_store_id || !onboarding?.tn_api_token) {
    return NextResponse.json({ error: "Faltan datos de TiendaNube" }, { status: 400 });
  }

  // Si ya está provisionado, no volver a crear
  ...
  const emailSlug = (client?.email || userId).split("@")[0].toLowerCase().replace(/[^a-z0-9]/g, "-");
  const clientId = `${emailSlug}-${Date.now().toString(36)}`;
  const nombreMarca = client?.name || emailSlug;
  const tnStoreId = onboarding.tn_store_id;
  const tnApiToken = onboarding.tn_api_token;
  const supabaseAnon = process.env.NOVA_SUPABASE_ANON_KEY!;

  try {
    // Crear los 3 workflows
    const [w1, w2, w3] = await Promise.all([
      n8nRequest("/workflows", "POST", buildWebhookTracking(clientId, supabaseAnon)),
      n8nRequest("/workflows", "POST", buildRecuperador(clientId, tnStoreId, tnApiToken, nombreMarca, supabaseAnon)),
    ...
    return NextResponse.json({ success: true, clientId, workflows: { tracking: w1.id, recuperador: w2.id, conversiones: w3.id } });
  } catch (err: unknown) {
